...
%             plot(D90,time_secs/24/3600,'k--','LineWidth',1.5);
            ...
            if saveFigs
                ...
                pos = get(gcf,'Position');
                set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
                print(sprintf('%s/CIP-ND-Temp_%ds/%s_CIP_ND-Temp_%ds_S%02d%s%s',saveDir,avgTime,flight,avgTime,ix,fNameAppnd,scaleStr),Ftype,Fres)
            end
        end
    end
    if plotNDtempBinned
    ...
    if plotLmdaTemp
        for ix = loopVctr
            ...
            lmda = cip_igf_nml.(sprlNames{ix})(:,3);
            tempCsprl = sDistF.tempC_avg.(sprlNames{ix});
            
            lmda_60s = arrayfun(@(i) nanmean(lmda(i:i+6-1)),1:6:length(lmda)-6+1)';
            tempC_60s = arrayfun(@(i) nanmean(tempCsprl(i:i+6-1)),1:6:length(tempCsprl)-6+1)';
            
            
            if saveFigs && noDisp
                figure('visible','off','Position', [10,10,750,1000]);
            else
            ...
            plot(lmda,tempCsprl,'o','MarkerFaceColor','b','MarkerEdgeColor','w','MarkerSize',10,'LineWidth',1.5);
            hold on
            plot(lmda_60s,tempC_60s,'k-','Linewidth',3);
            grid on
            xlabel('Lambda (cm^{-1})');
            ylabel(sprintf('T (%cC)', char(176)));
            set(ax,'XMinorTick','on');
            ...
            % Plot ML top/bottom locations and annotate with temp
            topStr = sprintf('%.2f %cC',mlTopTemp(ix),char(176));
            botStr = sprintf('%.2f %cC',mlBotTemp(ix),char(176));

            labLocFac = 0.92;
            plot([0 90], [1 1]*mlTopTemp(ix),'k--')
            tMT = text(90*labLocFac,mlTopTemp(ix),topStr,'HorizontalAlignment','center','BackgroundColor','k','color','w');
            plot([0 90], [1 1]*mlBotTemp(ix),'k--')
            tMB = text(90*labLocFac,mlBotTemp(ix),botStr,'HorizontalAlignment','center','BackgroundColor','k','color','w');
            hold off
            
            title([flight ' - Spiral ' num2str(ix) ' - CIP Fit Lambda']);
            ...
            if saveFigs
                ...
                pos = get(gcf,'Position');
                set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
                print(sprintf('%s/%s_CIP_Lambda-Temp_60s_S%02d%s',saveDir,flight,ix,fNameAppnd),Ftype,Fres)
            end
        end
    end
    ...
    if plotMDtemp
        ...
        for ix = loopVctr
            ...
            if saveFigs
                ...
                pos = get(gcf,'Position');
                set(gcf,'PaperPositionMode','Auto','PaperUnits','Inches','PaperSize',[pos(3), pos(4)])
                print(sprintf('%s/CIP-MD-Temp_%ds/%s_CIP_MD-Temp_%ds_S%02d%s%s',saveDir,avgTime,flight,avgTime,ix,fNameAppnd,scaleStr),Ftype,Fres)
            end
        end
    end
    if plotMDtempBinned
