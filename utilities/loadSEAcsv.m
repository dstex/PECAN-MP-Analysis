...
%SEAfile = '/Users/danstechman/GoogleDrive/PECAN-Data/mp-data/20150706/20150706_SEA_TWC.txt';
delimiter = ',';
startRow = 2;
...
TimeUTC = dates{:, 1};
SEA_TWC = cell2mat(rawNumericColumns(:, 1));

% For code requiring serial dates (datenum) instead of datetime, uncomment the following line(s) below to return the imported
% dates as datenum(s).

% TimeUTC=datenum(TimeUTC);

%% Convert datetime to seconds since midnight
SEA_Time = round(86400*mod(datenum(TimeUTC),1));

%% Clear temporary variables
clearvars delimiter startRow formatSpec fileID dataArray ans raw col numericData rawData row regexstr...
	result numbers invalidThousandsSeparator thousandsRegExp me dates blankDates anyBlankDates invalidDates anyInvalidDates rawNumericColumns R TimeUTC;