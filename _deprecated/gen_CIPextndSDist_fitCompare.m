...
if saveFigs
	...
	if (exist([saveDir '/expFit/10sFits'], 'dir') ~= 7)
		mkdir([saveDir '/expFit/10sFits'])
	end
end
...
if ~allSprls
	switch flight
		case '20150706'
			loopVctr = [1]; % Spiral numbers to plot
	end
else
...
for ix = loopVctr
	...
	cipConc_ext_old.(sprlNames{ix}) = NaN(size(cipConc,1),numExt_bins); % N(D) using only the exponential fit as a basis
	cipConc_ext_new.(sprlNames{ix}) = NaN(size(cipConc,1),numExt_bins); % N(D) using only the exponential fit as a basis
	...
	%}
	cip_coeff_all_old.(sprlNames{ix}) = NaN(size(cipConc,1),2);
	cipPSDobsFit_old.(sprlNames{ix}) = NaN(size(cipConc));
% 	gof_new.(sprlNames{ix}) = struct;
	
	cip_coeff_all_new.(sprlNames{ix}) = NaN(size(cipConc,1),2);
	cipPSDobsFit_new.(sprlNames{ix}) = NaN(size(cipConc));
	chisquare_new.(sprlNames{ix}) = NaN(size(cipConc,1),1);
...
% 	for ii = 10
		...
		if (all(isnan(cipConc(ii,:))) || length(find(cipConc(ii,:) > 0)) < 3)
			...
			cip_coeff_all_old.(sprlNames{ix})(ii,:) = NaN;
			cipPSDobsFit_old.(sprlNames{ix})(ii,:) = NaN;
			gof_new.(sprlNames{ix})(ii) = struct('sse',{NaN},'rsquare',{NaN},'dfe',{NaN},'adjrsquare',{NaN},'rmse',{NaN});
			
			cip_coeff_all_new.(sprlNames{ix})(ii,:) = NaN;
			cipPSDobsFit_new.(sprlNames{ix})(ii,:) = NaN;
			chisquare_new.(sprlNames{ix})(ii) = NaN;
			
			fitSkip = fitSkip+1;
			fileTail = '_lt3';
		else		
% 			I = find(cipPSD > 0); % Values to include in calculation
% 			E = find(cipPSD <= 0); % Values to exclude (set to NaN)
% 			cipPSDobsFit.(sprlNames{ix})(ii,E) = NaN;
% 			[cip_coeff_all.(sprlNames{ix})(ii,:), cipPSDobsFit.(sprlNames{ix})(ii,:), ~, ~] = createExpFit(cip_binMid, cipPSD');
			[cip_coeff_all_new.(sprlNames{ix})(ii,:), cipPSDobsFit_new.(sprlNames{ix})(ii,:), chisquare_new.(sprlNames{ix})(ii)] = expFitpecan(cip_binMid, cipPSD');
			[cip_coeff_all_old.(sprlNames{ix})(ii,:), cipPSDobsFit_old.(sprlNames{ix})(ii,:), fitresult, gof_new.(sprlNames{ix})(ii)] = createExpFit(cip_binMid, cipPSD');
				
			if (cip_coeff_all_old.(sprlNames{ix})(ii,2) < 0 || cip_coeff_all_new.(sprlNames{ix})(ii,2) < 0)
		...
		n0_old = cip_coeff_all_old.(sprlNames{ix})(ii,1);
		lmda_old = cip_coeff_all_old.(sprlNames{ix})(ii,2);
		n0_new = cip_coeff_all_new.(sprlNames{ix})(ii,1);
		lmda_new = cip_coeff_all_new.(sprlNames{ix})(ii,2);

		cipConc_hybrid.(sprlNames{ix})(ii,length(cip_binMid)+1:end) = n0_new.*exp(-lmda_new.*cipExt_binMid(length(cip_binMid)+1:end));
		cipConc_ext_old.(sprlNames{ix})(ii,:) = n0_old.*exp(-lmda_old.*cipExt_binMid);
		cipConc_ext_new.(sprlNames{ix})(ii,:) = n0_new.*exp(-lmda_new.*cipExt_binMid);
		...
		if plotNDintvl
			...
			hold on
			plot(cipExt_binMin*10,cipConc_ext_new.(sprlNames{ix})(ii,:),'b','LineWidth', 1.5);
% 			plot(cip_binMin*10,cipPSDobsFit_new.(sprlNames{ix})(ii,:),'b','LineWidth', 1.5);
			plot(cipExt_binMin*10,cipConc_ext_old.(sprlNames{ix})(ii,:),'k','LineWidth', 1.5);
% 			plot(cip_binMin*10,cipPSDobsFit_old.(sprlNames{ix})(ii,:),'k','LineWidth', 1.5);
			title([flight ' - CIP Exp. Fit - Spiral ' num2str(ix) titlStr ' - ' num2str(cipTime) ' - ' num2str(ii)],'FontSize',24);
			
			lgnd = legend('CIP Obs','CIP New expFit','CIP Old expFit');
			lgnd.FontSize = 18;
			xlabel('D [mm]');
			ylabel('N(D) [cm^{-4}]');
			set(gca,'yscale','log');
			...
			if saveFigs
				...
				print([saveDir '/expFit/10sFits/' flight '_expFit-CIP_S' num2str(ix) '-' sprintf('%2.2d',ii) '-' num2str(cipTime) fileStr fileTail],Ftype,'-r0')
			end
		end
	end
	...
	if plotND
		...
		cipObsFit = cipPSDobsFit_old.(sprlNames{ix});
		...
		plot(cip_binMin*10,nanmean(cipObsFit,1),'b','LineWidth', 1.5);
